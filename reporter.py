...
from ace_metrics.alerts.users import get_all_users, generate_user_alert_stats, alert_quantities_by_user_by_month

from ace_metrics.alerts.alert_types import (
...
from ace_metrics.events import (
...
def send_email_notification(
    smtp_config: configparser.SectionProxy,
...
def email_reports_based_on_configuration(
    config: configparser.ConfigParser, report_context_map: dict, approved_reports: list, archive=False
):
...
def create_high_level_report(
    ...
    logging.info(
        f"generating high level report: alert data('{start_date}' and '{end_date}') and event data('{event_start_date}' and '{event_end_date}')"
    )
    ...
    for stat in VALID_ALERT_STATS:
        ...
        if alert_stat_map_bh:
            if "time" in stat:
                ...
                alert_stat_report_map[f"{stat}-BH"] = {
                    "table": alert_stat_map_bh[stat],
        ...
        alert_stat_report_map[stat] = {
            "table": alert_stat_map[stat],
    ...
    alert_category_quantities = alert_type_quantities_by_category_by_month(
        start_date, end_date, db, alert_type_categories_key
    )
    alert_category_quantities.description = (
        "Alert types can vary on an individual hunt "
    ...
    alert_stat_report_map["alert-type-quantities"] = {
        "table": alert_category_quantities,
    ...
    event_query = EVENT_COUNT_TIME_DB_QUERY.replace("{<[TIME_FORMAT]>}", time_format).replace(
    ...
    raw_event_data_start_date = event_end_date - relativedelta(
    ...
    report_html = template.render(
        alert_stat_report_map=alert_stat_report_map,
...
def main():
    ...
    if "analysts" in args.approved_reports:
        ...
        overall_analyst_tables["analyst-alert-quantities"] = {
            "table": user_dispositions_per_month,
            "html_plot": generate_html_plot(user_dispositions_per_month),
        }
        ...
        for user_id in user_ids:
            ...
            for stat in VALID_ALERT_STATS:
                all_user_stat_map[user_id][
                    stat
                ].name = f"{users[user_id]['display_name']}: {FRIENDLY_STAT_NAME_MAP[stat]}"
                if all_user_stat_map_bh is not None:
                    if "time" in stat:
                        all_user_stat_map_bh[user_id][
                            stat
                        ].name = f"{users[user_id]['display_name']}: {FRIENDLY_STAT_NAME_MAP[stat]} (Business Hours)"
                        analyst_stat_report_map[user_id][f"{stat}-BH"] = {
                            "table": all_user_stat_map_bh[user_id][stat],
                            "html_plot": generate_html_plot(all_user_stat_map_bh[user_id][stat]),
                        }
                        report_context_map[report_type][zip_report_file_name].append(
                            f"{users[user_id]['display_name']}: {all_user_stat_map_bh[user_id][stat].name}"
                        )
                        tables_for_xlsx.append(all_user_stat_map_bh[user_id][stat])
                analyst_stat_report_map[user_id][stat] = {
                    "table": all_user_stat_map[user_id][stat],
                    "html_plot": generate_html_plot(all_user_stat_map[user_id][stat]),
                }

                report_context_map[report_type][zip_report_file_name].append(
                    f"{users[user_id]['display_name']}: {FRIENDLY_STAT_NAME_MAP[stat]}"
                )
        ...
        if template is None:
            ...
            return False
        report_html = template.render(
            analyst_stat_report_map=analyst_stat_report_map,
        ...
        for user_id in user_ids:
            ...
            analyst_report = analyst_template.render(
                analyst_stat_report_map=analyst_stat_report_map,
                overall_analyst_tables=overall_analyst_tables,
                analyst_report_file_map=analyst_report_file_map,
                users=users,
